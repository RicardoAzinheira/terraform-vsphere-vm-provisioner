...
// VM on a single Host and a single Datastore, with Linux and automatic (DHCP) IP addressing.
...
// Module
//
module "VM-OnHost-OnDataStore-Linux-ip_addressing_auto" {
...
// Module outputs
//
output "VM-OnHost-OnDataStore-Linux-ip_addressing_auto-IP-Address" {
  value = module.VM-OnHost-OnDataStore-Linux-ip_addressing_auto.VM-OnHost-OnDataStore-Linux-ip_addressing_auto-IPaddress
}