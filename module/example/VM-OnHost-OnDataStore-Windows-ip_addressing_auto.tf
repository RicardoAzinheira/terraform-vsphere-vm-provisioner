...
// VM on a single Host and a single Datastore, with Windows and automatic (DHCP) IP addressing.
...
//
locals {
  compute_type                           = "host"                       // Acceptable values: "cluster" for compute cluster, or "host" for single ESXi host.
  storage_type                           = "datastore"                  // Acceptable values: "datastore" for single datastore or vSAN volume, or "datastore_cluster".
  os_type                                = "windows"                    // Acceptable values: "Windows or "Linux"
  ip_addressing_type                     = "auto"                       // Acceptable values: "manual" for manually defined network values, or "auto" for DHCP configuration.
}

// Module
//
module "VM-OnHost-OnDataStore-Windows-ip_addressing_auto" {
  source                                 = "../"                        // This should point to the module's location, on git or localy.
...
// Module outputs
//
output "VM-OnHost-OnDataStore-Windows-ip_addressing_auto-IP-Address" {
  value = module.VM-OnHost-OnDataStore-Windows-ip_addressing_auto.VM-OnHost-OnDataStore-Windows-ip_addressing_auto-IPaddress
}